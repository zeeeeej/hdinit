...
static int parse_cmd(int sock_fd, const char *buffer, size_t size)
{
    ...
    else if (strcmp("progress", cmd) == 0)
    {
        ...
        if (progress_item == NULL || !cJSON_IsNumber(progress_item))
        {
            fprintf(stderr, "Failed to parse 'progress' object\n");
            cJSON_Delete(root);
            return -1;
        }else{
            if (g_progress >= 100)
            ...
            hd_print_progress_bar(g_progress);
        }
    }
    else
    {
